...
type range = Ascending | Descending | Invalid
...
let rec range_des a b = if a < b then [] else a :: range_des (a - 1) b
let rec range_asc a b = if a > b then [] else a :: range_asc (a + 1) b
...
let pretty_print_int i =
  let formatted_int =
    if i < parsed_args.(1) then string_of_int i ^ "," else string_of_int i
  in
  print_string formatted_int

let main = List.iter pretty_print_int rangey
let () = main
